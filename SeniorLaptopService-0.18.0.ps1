...
$zapretUpdate = {
    ...
    $youtubeTestUrl = 'https://www.youtube.com/@YouTube'
    Write-ServiceLog "Opening YouTube verification page: $youtubeTestUrl"
    Start-Process $youtubeTestUrl
    [Windows.Forms.MessageBox]::Show("Zapret установлен и запущен.`n`nСтратегия №9: $($strategyResult.registered)`nСлужба: $($strategyResult.service_status)`n`nОткрыт официальный канал YouTube. Проверьте, что страница и видео загружаются.",'Проверка Zapret','OK','Information') | Out-Null
}
...
<div class="step"><span class="num">10. Zapret: установить / обновить</span><p>Скачивает официальный релиз, автоматически выбирает стратегию №9, устанавливает и проверяет службу, затем открывает официальный канал YouTube. В браузере убедитесь, что страница и видео действительно загружаются. Если проверка не прошла, откройте управление Zapret и проведите диагностику.</p></div>
