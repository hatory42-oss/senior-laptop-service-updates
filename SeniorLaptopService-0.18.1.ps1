...
function Set-SystemMasterVolume([single]$Level) {
    if ($Level -lt 0 -or $Level -gt 1) { throw 'Уровень громкости должен быть от 0 до 1.' }
    if ($null -eq ('SeniorLaptopServiceAudio.VolumeControl' -as [type])) {
        Add-Type -TypeDefinition @'
using System;
using System.Runtime.InteropServices;
namespace SeniorLaptopServiceAudio {
    enum EDataFlow { eRender, eCapture, eAll }
    enum ERole { eConsole, eMultimedia, eCommunications }
    [ComImport, Guid("BCDE0395-E52F-467C-8E3D-C4579291692E")]
    class MMDeviceEnumerator { }
    [ComImport, InterfaceType(ComInterfaceType.InterfaceIsIUnknown), Guid("A95664D2-9614-4F35-A746-DE8DB63617E6")]
    interface IMMDeviceEnumerator { int NotImpl1(); [PreserveSig] int GetDefaultAudioEndpoint(EDataFlow dataFlow, ERole role, out IMMDevice device); }
    [ComImport, InterfaceType(ComInterfaceType.InterfaceIsIUnknown), Guid("D666063F-1587-4E43-81F1-B948E807363F")]
    interface IMMDevice { [PreserveSig] int Activate(ref Guid iid, int clsCtx, IntPtr activationParams, [MarshalAs(UnmanagedType.IUnknown)] out object interfacePointer); }
    ...
    interface IAudioEndpointVolume {
        ...
        int SetMasterVolumeLevel(float levelDb, Guid context); int SetMasterVolumeLevelScalar(float level, Guid context);
        int GetMasterVolumeLevel(out float levelDb); int GetMasterVolumeLevelScalar(out float level);
        int SetChannelVolumeLevel(uint channel, float levelDb, Guid context); int SetChannelVolumeLevelScalar(uint channel, float level, Guid context);
        int GetChannelVolumeLevel(uint channel, out float levelDb); int GetChannelVolumeLevelScalar(uint channel, out float level);
    ...
    public static class VolumeControl {
        public static void Set(float level) {
            IMMDeviceEnumerator enumerator = (IMMDeviceEnumerator)(new MMDeviceEnumerator()); IMMDevice device;
            ...
            Marshal.ThrowExceptionForHR(endpoint.SetMute(false, Guid.Empty)); Marshal.ThrowExceptionForHR(endpoint.SetMasterVolumeLevelScalar(level, Guid.Empty));
        }
    }
}
'@
    }
    [SeniorLaptopServiceAudio.VolumeControl]::Set($Level)
}
...
function Install-ZapretStrategyNine([string]$RootPath) {
    ...
    if ($strategyFiles.Count -lt 9) { throw "В релизе Zapret найдено только $($strategyFiles.Count) стратегий; выбрать стратегию №9 невозможно." }
    $selectedStrategy = $strategyFiles[8].Name
    Write-ServiceLog "Zapret: automatic service installation, strategy #9 = $selectedStrategy"
    $startInfo = [Diagnostics.ProcessStartInfo]::new()
    $startInfo.FileName = 'cmd.exe'
    $startInfo.Arguments = '/d /c ""' + $serviceBat + '" admin"'
    ...
    try {
        ...
        $process.StandardInput.WriteLine('9')
        $process.StandardInput.Write('X')
        $process.StandardInput.WriteLine('0')
        $process.StandardInput.Close()
        if (-not $process.WaitForExit(120000)) { $process.Kill(); throw 'Установка стратегии Zapret не завершилась за 120 секунд.' }
        if ($process.ExitCode -ne 0) { throw "service.bat завершился с кодом $($process.ExitCode)." }
    } finally { $process.Dispose() }
    Start-Sleep -Seconds 2
    $service = Get-Service -Name 'zapret' -ErrorAction SilentlyContinue
    if ($null -eq $service -or $service.Status -ne 'Running') { throw 'Служба zapret не запущена после автоматической установки.' }
    $installedStrategy = [string](Get-ItemPropertyValue -LiteralPath 'HKLM:\System\CurrentControlSet\Services\zapret' -Name 'zapret-discord-youtube' -ErrorAction SilentlyContinue)
    Write-ServiceLog "Zapret service is running. Requested: $selectedStrategy; registered: $installedStrategy"
    [ordered]@{ requested=$selectedStrategy; registered=$installedStrategy; service_status=$service.Status.ToString() }
}
...
$form.Size = New-Object Drawing.Size(820,790)
$form.StartPosition = 'CenterScreen'
$form.MinimumSize = New-Object Drawing.Size(760,720)
...
$buttons.Dock = 'Top'; $buttons.Height = 415; $buttons.ColumnCount = 3; $buttons.RowCount = 6; $buttons.Padding = New-Object Windows.Forms.Padding(8,4,8,4)
...
1..6 | ForEach-Object { [void]$buttons.RowStyles.Add((New-Object Windows.Forms.RowStyle([Windows.Forms.SizeType]::Percent,16.66))) }
...
$zapretUpdate = {
    ...
    Write-ServiceLog "Zapret $($release.tag_name) installed from official GitHub release. Applying strategy #9 automatically."
    $strategyResult = Install-ZapretStrategyNine $ZapretRoot
    try { Set-SystemMasterVolume 0.05; Write-ServiceLog 'System volume set to 5% for the YouTube test.' } catch { Write-ServiceLog "Could not lower test volume: $($_.Exception.Message)" }
    $youtubeTestUrl = 'https://www.youtube.com/watch?v=LNrBbGcLhXg&autoplay=1'
    Write-ServiceLog "Opening YouTube verification page: $youtubeTestUrl"
    Start-Process $youtubeTestUrl
    [Windows.Forms.MessageBox]::Show("Zapret установлен и запущен.`n`nСтратегия №9: $($strategyResult.registered)`nСлужба: $($strategyResult.service_status)`nГромкость установлена на 5%.`n`nОткрыто тестовое видео с котиками. Проверьте воспроизведение. Если браузер запретил автозапуск, нажмите ▶ один раз.",'Проверка Zapret','OK','Information') | Out-Null
}
...
<div class="step"><span class="num">7. Удалить Avast</span><p>Нажимайте только если аудит или отчёт подтвердил наличие Avast. Windows Defender должен остаться включённым.</p></div>
...
<div class="step"><span class="num">10. Zapret: установить / обновить</span><p>Скачивает официальный релиз, автоматически выбирает стратегию №9, устанавливает и проверяет службу, уменьшает громкость до 5% и открывает тестовое видео с котиками. Убедитесь, что видео воспроизводится. Если браузер запретил автозапуск, нажмите кнопку воспроизведения один раз. Если проверка не прошла, откройте управление Zapret и проведите диагностику.</p></div>
...
<h2>Рекомендуемая последовательность для обычного обслуживания</h2><p><b>Обновление → Аудит → Отчёт GitHub → Анализ → Очистка → исправление Яндекса/удаление Avast при необходимости → Оптимизация → блокировщик рекламы → Zapret → Еженедельная задача → Памятка.</b></p>
...
$engineerButton = Add-ActionButton 'Открыть инструкцию сервисного инженера' $engineerGuideAction 0 5 ([Drawing.Color]::LightYellow)
$buttons.SetColumnSpan($engineerButton,2)
Add-ActionButton 'Установить блокировщик рекламы' $installAdBlockAction 2 5 ([Drawing.Color]::Honeydew)
