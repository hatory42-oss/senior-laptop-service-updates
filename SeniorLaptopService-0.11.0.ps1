...
function Send-ReportByGmail([string]$ReportPath, [string]$AppPassword) {
    $from = 'hatory42@gmail.com'; $to = 'hatory42@gmail.com'
    $message = New-Object Net.Mail.MailMessage
    $message.From=$from; [void]$message.To.Add($to)
    $message.Subject='SeniorLaptopService report - ' + $env:COMPUTERNAME + ' - ' + (Get-Date -Format 'yyyy-MM-dd HH:mm')
    $message.Body="Автоматический технический отчёт сервисной панели.`r`nКомпьютер: $env:COMPUTERNAME`r`nПароли Wi-Fi, cookies, история и личные файлы не включены."
    $attachment = New-Object Net.Mail.Attachment($ReportPath); [void]$message.Attachments.Add($attachment)
    $smtp = New-Object Net.Mail.SmtpClient('smtp.gmail.com',587)
    $smtp.EnableSsl=$true; $smtp.Credentials=New-Object Net.NetworkCredential($from,$AppPassword)
    try { $smtp.Send($message) } finally { $attachment.Dispose(); $message.Dispose(); $smtp.Dispose() }
}
...
function Add-ActionButton([string]$Text, [scriptblock]$Action, [int]$Column, [int]$Row, [Drawing.Color]$Color = [Drawing.Color]::WhiteSmoke) {
    ...
    $button.Add_Click({
        ...
        catch { Write-ServiceLog ('ERROR: ' + $_.Exception.Message); [Windows.Forms.MessageBox]::Show($_.Exception.Message,'Ошибка','OK','Error') | Out-Null }
...
$audit = {
    ...
    $avast = Get-ItemProperty 'HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\*','HKLM:\Software\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\*' -ErrorAction SilentlyContinue | Where-Object DisplayName -match 'Avast'
...
$removeAvast = {
    $items = @(Get-ItemProperty 'HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall\*','HKLM:\Software\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\*' -ErrorAction SilentlyContinue | Where-Object DisplayName -match 'Avast')
...
$reportAction = {
    ...
    $password = Show-PasswordPrompt 'Отправка отчёта' 'Введите пароль приложения Google для hatory42@gmail.com. Пароль используется один раз и не сохраняется.'
    if ([string]::IsNullOrWhiteSpace($password)) { Write-ServiceLog "Report saved locally; email cancelled: $reportPath"; return }
    Write-ServiceLog 'Sending report to hatory42@gmail.com via Gmail SMTP...'
    Send-ReportByGmail $reportPath $password
    $password=$null
    Write-ServiceLog "Report sent successfully; local copy: $reportPath"
    [Windows.Forms.MessageBox]::Show("Отчёт отправлен на hatory42@gmail.com`n`nЛокальная копия:`n$reportPath",'Отправлено') | Out-Null
}
...
Add-ActionButton 'Отправить отчёт на почту' $reportAction 2 2 ([Drawing.Color]::Lavender)
