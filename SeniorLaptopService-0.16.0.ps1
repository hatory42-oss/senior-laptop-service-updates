...
<div class="step"><span class="num">9. Zapret: установить / обновить</span><p>Скачивает официальный релиз и обновляет списки. После этого через «Открыть управление» проверьте запуск службы и доступ к YouTube.</p></div>
<div class="step"><span class="num">10. Установить еженедельную очистку</span><p>Делайте после завершения настройки. Каждое воскресенье в 12:00 очистка запускается от SYSTEM и обновляет списки Zapret. Повторное нажатие безопасно: задача заменяется новой версией.</p></div>
...
<h2>Рекомендуемая последовательность для обычного обслуживания</h2><p><b>Обновление → Аудит → Отчёт GitHub → Анализ → Очистка → исправление Яндекса/удаление Avast при необходимости → Оптимизация → Zapret → Еженедельная задача → Памятка.</b></p>
...
$buttons.SetColumnSpan($engineerButton,3)
