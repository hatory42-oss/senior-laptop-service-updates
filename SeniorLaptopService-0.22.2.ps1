...
function Install-ZapretStrategy([string]$RootPath, [int]$StrategyIndex) {
    ...
    try {
        ...
        if (-not $installedInTime) {
            ...
            throw "Стратегия Zapret №$StrategyIndex не прошла практическую проверку за 60 секунд. Служба: $lastServiceState; PID: $lastServicePid; PathName: $lastServicePath; установщик: $installerState."
        }
