...
function Install-ZapretStrategy([string]$RootPath, [int]$StrategyIndex) {
    ...
    try {
        ...
        if (-not $installedInTime) {
            ...
            $pathPreview = if ($registryImagePath.Length -gt 400) { $registryImagePath.Substring(0,400) + '...[сокращено]' } else { $registryImagePath }
    ...
    $registeredServicePath = [string]$service.PathName
