...
function Remove-ZapretInstallation {
    ...
    if (Test-Path -LiteralPath $ZapretRoot) {
    ...
    Write-ServiceLog 'Previous Zapret service and files removed.'
}
...
function Install-ZapretStrategy([string]$RootPath, [int]$StrategyIndex) {
    ...
    $automaticText = [Regex]::Replace($serviceText,$completionPattern,$completionReplacement,1)
    if ($automaticText -eq $serviceText) { throw 'Не удалось подготовить автоматический режим service.bat: структура файла изменилась.' }
    ...
    try {
        ...
        $process.StandardInput.WriteLine([string]$StrategyIndex)
        $process.StandardInput.Close()
        $expectedRegistryValue = [IO.Path]::GetFileNameWithoutExtension($selectedStrategy)
        $installedInTime = $false
        $installTimer = [Diagnostics.Stopwatch]::StartNew()
        while ($installTimer.Elapsed.TotalSeconds -lt 60) {
            [Windows.Forms.Application]::DoEvents()
            $currentService = Get-Service -Name 'zapret' -ErrorAction SilentlyContinue
            $currentStrategy = [string](Get-ItemPropertyValue -LiteralPath 'HKLM:\System\CurrentControlSet\Services\zapret' -Name 'zapret-discord-youtube' -ErrorAction SilentlyContinue)
            if ($null -ne $currentService -and $currentService.Status -eq 'Running' -and $currentStrategy -eq $expectedRegistryValue) { $installedInTime = $true; break }
            if ($process.HasExited -and $process.ExitCode -ne 0) { throw "service.bat завершился с кодом $($process.ExitCode)." }
            Start-Sleep -Milliseconds 250
        }
        $installTimer.Stop()
        if (-not $process.HasExited) { try { $process.Kill() } catch { } }
        if (-not $installedInTime) { throw "Стратегия Zapret №$StrategyIndex не была запущена за 60 секунд." }
    ...
    [ordered]@{ index=$StrategyIndex; requested=$selectedStrategy; registered=$installedStrategy; service_status=$service.Status.ToString(); total=$strategyFiles.Count }
}
...
function Test-YouTubeMediaStream([string]$VideoId = 'LNrBbGcLhXg', [int]$TimeoutSeconds = 25) {
    try {
        ...
        if (-not $keyMatch.Success -or -not $versionMatch.Success) { Write-ServiceLog 'YouTube stream test: player API parameters not found.'; return $false }
        ...
        if ([string]$player.playabilityStatus.status -ne 'OK') { Write-ServiceLog "YouTube stream test: playability=$($player.playabilityStatus.status)."; return $false }
        ...
        if ([string]::IsNullOrWhiteSpace($mediaUrl)) { Write-ServiceLog 'YouTube stream test: direct media URL not supplied.'; return $false }
...
function Invoke-YouTubeVisualTest([string]$Url, [int]$DurationSeconds = 45) {
    $edgeCandidates = New-Object Collections.Generic.List[string]
    if (-not [string]::IsNullOrWhiteSpace(${env:ProgramFiles(x86)})) { $edgeCandidates.Add((Join-Path ${env:ProgramFiles(x86)} 'Microsoft\Edge\Application\msedge.exe')) }
    if (-not [string]::IsNullOrWhiteSpace($env:ProgramFiles)) { $edgeCandidates.Add((Join-Path $env:ProgramFiles 'Microsoft\Edge\Application\msedge.exe')) }
    $edgeCandidates = @($edgeCandidates | Where-Object { Test-Path -LiteralPath $_ })
    $browserExe = $edgeCandidates | Select-Object -First 1
    if ([string]::IsNullOrWhiteSpace($browserExe)) { throw 'Microsoft Edge не найден; автоматическую изолированную проверку YouTube выполнить невозможно.' }
    ...
    $connectionOk = $false
    try {
        Start-Process -FilePath $browserExe -ArgumentList @("--user-data-dir=`"$testProfile`"",'--new-window','--no-first-run','--autoplay-policy=no-user-gesture-required',$Url) | Out-Null
        Write-ServiceLog "YouTube test window opened for $DurationSeconds seconds. System audio settings are unchanged."
        ...
        $timer.Stop()
        Get-CimInstance Win32_Process -Filter "Name='msedge.exe'" -ErrorAction SilentlyContinue | Where-Object { [string]$_.CommandLine -like "*$testMarker*" } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue }
        Start-Sleep -Milliseconds 500
        try {
...
$zapretUpdate = {
    ...
    Write-ServiceLog "Zapret $($release.tag_name) installed cleanly from official GitHub release. Testing strategy #9 first."
    ...
    foreach ($strategyNumber in $strategyOrder) {
        ...
        $candidate = Install-ZapretStrategy $ZapretRoot $strategyNumber
        Start-Sleep -Seconds 3
        if (Test-YouTubeMediaStream 'LNrBbGcLhXg' 25) { $strategyResult = $candidate; Write-ServiceLog "Zapret strategy #$strategyNumber passed the media stream test."; break }
        Write-ServiceLog "Zapret strategy #$strategyNumber did not pass. Trying the next strategy automatically."
    }
    if ($null -eq $strategyResult) { throw "Ни одна из $strategyCount стратегий Zapret не обеспечила поток YouTube. Сохранена последняя стратегия; создан отчёт для анализа." }
    $youtubeTestUrl = 'https://www.youtube.com/watch?v=LNrBbGcLhXg&autoplay=1'
    Write-ServiceLog "Starting isolated YouTube verification: $youtubeTestUrl"
    [void](Invoke-YouTubeVisualTest $youtubeTestUrl 45)
    [Windows.Forms.MessageBox]::Show("Zapret установлен и запущен.`n`nРабочая стратегия №$($strategyResult.index): $($strategyResult.registered)`nСлужба: $($strategyResult.service_status)`n`nПоток YouTube подтверждён. Тестовое окно закрыто автоматически через 45 секунд. Системные звуки не изменялись.",'Проверка Zapret','OK','Information') | Out-Null
}
