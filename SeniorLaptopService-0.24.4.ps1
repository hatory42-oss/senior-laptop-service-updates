...
$BundledZapretRoot = Join-Path $PSScriptRoot 'Zapret'
...
$PortableLauncherFiles = @(
    [ordered]@{ Name='Run-As-Administrator.cmd'; Url='https://raw.githubusercontent.com/hatory42-oss/senior-laptop-service-updates/main/Run-As-Administrator.cmd'; Sha256='0633FA7F5D6ED286303BEDBD24FF49BFEF5F24447254EA7A0E25FE65792A16DF' },
    [ordered]@{ Name='Start-SeniorLaptopService.ps1'; Url='https://raw.githubusercontent.com/hatory42-oss/senior-laptop-service-updates/main/Start-SeniorLaptopService.ps1'; Sha256='083DCC3B6BC45A936C5C57D030ACEF0A70046CC9382FA3D84DF5959E1B5863DB' }
)
...
function Set-ManagedBrowserExtension([string]$PolicyPath, [string]$ExtensionId) {
    ...
    if ($ExtensionId -in $knownBlockers) {
        foreach ($property in @($policy.PSObject.Properties | Where-Object { $_.Name -match '^\d+$' })) {
            $configuredId = ([string]$property.Value -split ';')[0]
            if ($configuredId -in $knownBlockers -and $configuredId -ne $ExtensionId) { Remove-ItemProperty -Path $PolicyPath -Name $property.Name -Force -ErrorAction SilentlyContinue }
        }
    }
    $policy = Get-ItemProperty -Path $PolicyPath -ErrorAction SilentlyContinue
    foreach ($property in @($policy.PSObject.Properties | Where-Object { $_.Name -match '^\d+$' })) {
        if (([string]$property.Value -split ';')[0] -eq $ExtensionId) { return }
    }
    $usedSlots = @($policy.PSObject.Properties | Where-Object { $_.Name -match '^\d+$' } | ForEach-Object { [int]$_.Name })
    $slot = if ($usedSlots.Count -gt 0) { [int](($usedSlots | Measure-Object -Maximum).Maximum) + 1 } else { 1 }
...
function Disable-YandexBrowserAutostart([string[]]$RunKeyPaths, [string[]]$StartupFolderPaths) {
    $removed = New-Object Collections.Generic.List[string]
    $runKeys = New-Object Collections.Generic.List[string]
    if ($null -ne $RunKeyPaths -and $RunKeyPaths.Count -gt 0) {
        foreach ($key in $RunKeyPaths) { if (-not [string]::IsNullOrWhiteSpace($key)) { $runKeys.Add($key) } }
    } else {
        foreach ($key in @(
        ...
        foreach ($sidKey in @(Get-ChildItem 'Registry::HKEY_USERS' -ErrorAction SilentlyContinue | Where-Object { $_.PSChildName -match '^S-1-5-21-(\d+-){3}\d+$' })) {
            $runKeys.Add(('Registry::HKEY_USERS\{0}\Software\Microsoft\Windows\CurrentVersion\Run' -f $sidKey.PSChildName))
        }
    }
    foreach ($runKey in @($runKeys | Select-Object -Unique)) {
        ...
        foreach ($property in @($properties.PSObject.Properties | Where-Object { $_.Name -notlike 'PS*' })) {
            ...
            if ($command -match '(?i)Yandex[\\/]YandexBrowser[\\/]Application[\\/]browser\.exe') {
                Remove-ItemProperty -LiteralPath $runKey -Name $property.Name -Force -ErrorAction SilentlyContinue
                $removed.Add("$runKey :: $($property.Name)")
            }
        }
    }

    $startupFolders = New-Object Collections.Generic.List[string]
    if ($null -ne $StartupFolderPaths -and $StartupFolderPaths.Count -gt 0) {
        foreach ($folder in $StartupFolderPaths) { if (-not [string]::IsNullOrWhiteSpace($folder)) { $startupFolders.Add($folder) } }
    } else {
        foreach ($folder in @([Environment]::GetFolderPath('Startup'),[Environment]::GetFolderPath('CommonStartup'))) {
            if (-not [string]::IsNullOrWhiteSpace($folder)) { $startupFolders.Add($folder) }
        }
        foreach ($folder in @(Get-ChildItem 'C:\Users\*\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup' -Directory -Force -ErrorAction SilentlyContinue)) { $startupFolders.Add($folder.FullName) }
    }
    $shell = New-Object -ComObject WScript.Shell
    foreach ($folder in @($startupFolders | Select-Object -Unique)) {
        foreach ($shortcut in @(Get-ChildItem -LiteralPath $folder -Filter '*.lnk' -File -Force -ErrorAction SilentlyContinue)) {
            try {
                $target = [string]$shell.CreateShortcut($shortcut.FullName).TargetPath
                if ($target -match '(?i)Yandex[\\/]YandexBrowser[\\/]Application[\\/]browser\.exe') {
                    Remove-Item -LiteralPath $shortcut.FullName -Force
                    $removed.Add($shortcut.FullName)
                }
            } catch { Write-ServiceLog "Не удалось проверить ярлык автозапуска $($shortcut.FullName): $($_.Exception.Message)" }
        }
    }
    Write-ServiceLog "Яндекс Браузер: удалено записей запуска окна вместе с Windows: $($removed.Count). Компоненты обновления сохранены."
...
function Remove-ZapretInstallation([bool]$RemoveFiles = $true) {
    ...
    if ($RemoveFiles -and (Test-Path -LiteralPath $ZapretRoot)) {
        ...
        if ($resolvedRoot.Length -lt 8 -or (Split-Path $resolvedRoot -Leaf) -ne 'zapret' -or -not $resolvedRoot.StartsWith($resolvedInstallRoot + '\',[StringComparison]::OrdinalIgnoreCase)) { throw "Отказ удаления небезопасного пути Zapret: $resolvedRoot" }
...
function Get-ZapretServiceImagePath([object]$Service) {
    $cimPath = [string](Get-SafeProperty $Service 'PathName')
    if (-not [string]::IsNullOrWhiteSpace($cimPath)) { return $cimPath }
    try {
        return [string](Get-ItemProperty -LiteralPath 'HKLM:\System\CurrentControlSet\Services\zapret' -ErrorAction Stop).ImagePath
    } catch { return '' }
}
...
function Install-ZapretStrategy([string]$RootPath, [int]$StrategyIndex) {
    ...
    # Keep the upstream menu initialization calls (game/ipset/update status).
    # Only replace the interactive answer after initialization is complete.
    $menuChoicePattern = '(?m)^set /p menu_choice=.*$'
    $automaticText = [Regex]::Replace($serviceText,$menuChoicePattern,'set "menu_choice=1"',1)
    ...
    if ($automaticText -eq $serviceText -or $automaticText -notmatch '(?m)^set "menu_choice=1"\s*$' -or $automaticText -notmatch ('(?m)^set "choice=' + $StrategyIndex + '"\s*$') -or $automaticText -notmatch '(?m)^exit /b 0\s*$') { throw 'Не удалось подготовить автоматический режим service.bat: структура файла изменилась.' }
    ...
    try {
        ...
        $manualStartAt = $null
        $expectedServiceRoot = [IO.Path]::GetFullPath($RootPath).TrimEnd('\') + '\'
        ...
        while ($installTimer.Elapsed.TotalSeconds -lt 15) {
            ...
            if ($null -ne $currentService) {
                ...
                $lastServicePath = Get-ZapretServiceImagePath $currentService
                $lastExitCode = $currentService.ExitCode
                $lastServiceExitCode = $currentService.ServiceSpecificExitCode
                if ($lastServiceState -eq 'Running' -and $lastServicePid -gt 0 -and -not [string]::IsNullOrWhiteSpace($lastServicePath) -and $lastServicePath.IndexOf($expectedServiceRoot,[StringComparison]::OrdinalIgnoreCase) -ge 0) { $installedInTime = $true; break }
            }
        ...
        if (-not $installedInTime) {
            $finalService = Get-CimInstance Win32_Service -Filter "Name='zapret'" -ErrorAction SilentlyContinue
            if ($null -ne $finalService) {
                $lastServiceState = [string]$finalService.State
                $lastServicePid = [int64]$finalService.ProcessId
                $lastServicePath = Get-ZapretServiceImagePath $finalService
                $lastExitCode = $finalService.ExitCode
                $lastServiceExitCode = $finalService.ServiceSpecificExitCode
                if ($lastServiceState -eq 'Running' -and $lastServicePid -gt 0 -and -not [string]::IsNullOrWhiteSpace($lastServicePath) -and $lastServicePath.IndexOf($expectedServiceRoot,[StringComparison]::OrdinalIgnoreCase) -ge 0) { $installedInTime = $true }
            }
        }
        if (-not $installedInTime) {
            $installerState = if ($process.HasExited) { "завершён, код $($process.ExitCode)" } else { 'не завершён' }
            $pathPreview = if ($lastServicePath.Length -gt 400) { $lastServicePath.Substring(0,400) + '...[сокращено]' } else { $lastServicePath }
            throw "Стратегия Zapret №$StrategyIndex не запустила службу. State=$lastServiceState; PID=$lastServicePid; ImagePath=$pathPreview; Win32ExitCode=$lastExitCode; ServiceExitCode=$lastServiceExitCode; установщик: $installerState."
        }
    ...
    $expectedServiceRoot = [IO.Path]::GetFullPath($RootPath).TrimEnd('\') + '\'
    $registeredServicePath = Get-ZapretServiceImagePath $service
    if ([string]::IsNullOrWhiteSpace($registeredServicePath) -or $registeredServicePath.IndexOf($expectedServiceRoot,[StringComparison]::OrdinalIgnoreCase) -lt 0) {
        throw "Служба zapret зарегистрирована не из постоянной папки. Ожидалось: $expectedServiceRoot; получено: $registeredServicePath"
    }
    $installedStrategy = Get-ZapretRegisteredStrategy
    Write-ServiceLog "Zapret service is running from permanent folder $expectedServiceRoot. Requested: $selectedStrategy; registered: $installedStrategy"
    [ordered]@{ index=$StrategyIndex; requested=$selectedStrategy; registered=$installedStrategy; service_status=[string]$service.State; service_pid=[int64]$service.ProcessId; service_path=$registeredServicePath; total=$strategyFiles.Count }
}
...
function Sync-PortableLauncherFiles {
    $pending = @($PortableLauncherFiles | Where-Object {
        $destination = Join-Path $PSScriptRoot $_.Name
        if (-not (Test-Path -LiteralPath $destination -PathType Leaf)) { return $true }
        try { return ((Get-FileHash -Algorithm SHA256 -LiteralPath $destination).Hash -ne $_.Sha256) } catch { return $true }
    })
    if ($pending.Count -eq 0) { return }

    Write-ServiceLog 'Обновление совместимого запускателя Windows 7...'
    foreach ($file in $pending) {
        if (-not $file.Url.StartsWith('https://') -or $file.Sha256 -notmatch '^[A-Fa-f0-9]{64}$') { throw "Некорректные данные запускателя: $($file.Name)" }
        $temporaryPath = Join-Path $env:TEMP ('SeniorLaptopService-launcher-' + [guid]::NewGuid().ToString('N') + '.tmp')
        try {
            $bytes = Get-UpdateBytesWithFallback $file.Url 30 45
            [IO.File]::WriteAllBytes($temporaryPath,$bytes)
            $actualHash = (Get-FileHash -Algorithm SHA256 -LiteralPath $temporaryPath).Hash
            if ($actualHash -ne $file.Sha256) { throw "SHA-256 файла $($file.Name) не совпал: $actualHash" }
            Copy-Item -LiteralPath $temporaryPath -Destination (Join-Path $PSScriptRoot $file.Name) -Force
            Write-ServiceLog "Запускатель обновлён: $($file.Name)"
        } finally {
...
try { Sync-PortableLauncherFiles } catch { Write-ServiceLog "Не удалось обновить совместимый запускатель: $($_.Exception.Message)" }
...
$zapretUpdate = {
    if (-not (Test-Administrator)) { throw 'Запустите программу от имени администратора.' }
    Write-ServiceLog "Checking permanent Zapret folder: $ZapretRoot"
    [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
    $versionFile = Join-Path $ZapretRoot '.installed-version'
    $installedVersion = if (Test-Path -LiteralPath $versionFile) { [string](Get-Content -LiteralPath $versionFile -Raw -ErrorAction SilentlyContinue).Trim() } else { '' }
    $installedReady = ($installedVersion -eq $PinnedZapretVersion -and (Test-Path -LiteralPath (Join-Path $ZapretRoot 'service.bat')) -and (Test-Path -LiteralPath (Join-Path $ZapretRoot 'bin\winws.exe')))
    $bundledVersionFile = Join-Path $BundledZapretRoot '.installed-version'
    $bundledVersion = if (Test-Path -LiteralPath $bundledVersionFile) { [string](Get-Content -LiteralPath $bundledVersionFile -Raw -ErrorAction SilentlyContinue).Trim() } else { '' }
    $bundledReady = (-not $BundledZapretRoot.Equals($ZapretRoot,[StringComparison]::OrdinalIgnoreCase) -and $bundledVersion -eq $PinnedZapretVersion -and (Test-Path -LiteralPath (Join-Path $BundledZapretRoot 'service.bat')) -and (Test-Path -LiteralPath (Join-Path $BundledZapretRoot 'bin\winws.exe')))
    if ($installedReady) {
        Write-ServiceLog "Installed Zapret $installedVersion is current; archive download and file replacement skipped."
        Remove-ZapretInstallation $false
    } else {
        Remove-ZapretInstallation $true
        Ensure-Directory $ZapretRoot
        if ($bundledReady) {
            Write-ServiceLog "Copying bundled Zapret $bundledVersion from $BundledZapretRoot to permanent folder $ZapretRoot..."
            Copy-Item (Join-Path $BundledZapretRoot '*') $ZapretRoot -Recurse -Force
        } else {
            Write-ServiceLog "Installed Zapret version '$installedVersion' differs from required '$PinnedZapretVersion'; downloading verified replacement to the computer..."
        ...
        Write-ServiceLog "Zapret $PinnedZapretVersion installed permanently at $ZapretRoot."
    }
    ...
    if ($strategyCount -lt 9) { throw "В установленном релизе Zapret найдено только $strategyCount стратегий; стратегия №9 отсутствует." }
    $strategyResult = Install-ZapretStrategy $ZapretRoot 9
    Write-ServiceLog 'Strategy #9 service is running. Internal YouTube player API check is intentionally skipped because it produced false failures.'
    ...
    [Windows.Forms.MessageBox]::Show("Zapret установлен в постоянную папку и запущен именно из неё.`n`nПапка: $ZapretRoot`nРабочая стратегия №$($strategyResult.index): $($strategyResult.registered)`nСлужба: $($strategyResult.service_status)`n`nВидео проверено с отметок 03:00 и 04:27. Каждое тестовое окно закрыто автоматически. Системные звуки не изменялись.",'Проверка Zapret','OK','Information') | Out-Null
}
...
$installAdBlockAction = {
    if (-not (Test-Administrator)) { throw 'Запустите программу от имени администратора.' }
    Write-ServiceLog 'Настройка защиты и упрощённого интерфейса Chrome и Яндекс Браузера...'
    $adGuardId = 'bgnkhhnnamicmpeenaelnjfhikgbkllg'
    $trafficLightId = 'cfnpidifppmenkapgihekkeednfoenal'
    $adGuardUrl = "https://chromewebstore.google.com/detail/adguard-adblocker/$adGuardId"
    $trafficLightUrl = "https://chromewebstore.google.com/detail/trafficlight/$trafficLightId"
    $configured = New-Object Collections.Generic.List[string]
    ...
    if ($chromeInstalled) {
        Set-ManagedBrowserExtension 'HKLM:\Software\Policies\Google\Chrome\ExtensionInstallForcelist' $adGuardId
        Set-ManagedBrowserExtension 'HKLM:\Software\Policies\Google\Chrome\ExtensionInstallForcelist' $trafficLightId
        $chromePolicy = 'HKLM:\Software\Policies\Google\Chrome'
        if (-not (Test-Path -LiteralPath $chromePolicy)) { New-Item -Path $chromePolicy -Force | Out-Null }
        $chromeDwordPolicies = [ordered]@{
        ...
        foreach ($policy in $chromeDwordPolicies.GetEnumerator()) {
            New-ItemProperty -Path $chromePolicy -Name $policy.Key -Value ([int]$policy.Value) -PropertyType DWord -Force | Out-Null
        }
        $configured.Add('Google Chrome: защита и простой режим')
        Write-ServiceLog 'Chrome: AdGuard и TrafficLight назначены автоматически; уведомления, всплывающие окна, навязчивая реклама и фоновые процессы ограничены.'
    }

    $yandexCandidates = New-Object Collections.Generic.List[string]
    foreach ($candidate in @(
        ...
        if (-not [string]::IsNullOrWhiteSpace($candidate) -and (Test-Path -LiteralPath $candidate)) { $yandexCandidates.Add($candidate) }
    }
    foreach ($candidate in @(Get-ChildItem 'C:\Users\*\AppData\Local\Yandex\YandexBrowser\Application\browser.exe' -Force -ErrorAction SilentlyContinue)) {
        if ($candidate.FullName -notin $yandexCandidates) { $yandexCandidates.Add($candidate.FullName) }
    }
    $yandexInstalled = $yandexCandidates.Count -gt 0
    $yandexExtensionsConfirmed = $false
    if ($yandexInstalled) {
        $yandexPolicy = 'HKLM:\Software\Policies\YandexBrowser'
        if (-not (Test-Path -LiteralPath $yandexPolicy)) { New-Item -Path $yandexPolicy -Force | Out-Null }
        $backgroundRoot = Join-Path $env:ProgramData 'SeniorLaptopService'
        Ensure-Directory $backgroundRoot
        $backgroundPath = Join-Path $backgroundRoot 'yandex-simple-background.png'
        if (-not (Test-Path -LiteralPath $backgroundPath)) {
            $bitmap = New-Object Drawing.Bitmap(64,64)
            $graphics = [Drawing.Graphics]::FromImage($bitmap)
            try {
                $graphics.Clear([Drawing.Color]::FromArgb(242,246,250))
                $bitmap.Save($backgroundPath,[Drawing.Imaging.ImageFormat]::Png)
            } finally {
                $graphics.Dispose()
                $bitmap.Dispose()
            }
        }
        $yandexDwordPolicies = [ordered]@{
            DefaultNotificationsSetting = 2
            DefaultPopupsSetting = 2
            DownloadRestrictions = 1
            YandexAdblock = 1
            YandexAntishock = 1
            NtpNotificationsDisable = 1
            Education = 0
            WelcomeExperiments = 0
            FastSignup = 0
            EnableBackgroundAnimationHighResolution = 0
            WebGalleryRotation = 0
            YandexDisableBackgroundVideos = 1
            SidePanelMode = 2
            NeuroNtpTools = 0
            YandexVideoSummarization = 0
            YandexVideoQR = 0
            BackgroundModeEnabled = 0
            YandexAutoLaunchMode = 0
            YandexBackgroundModeDisableRestart = 1
            DefaultBrowserSettingEnabled = 0
        }
        foreach ($policy in $yandexDwordPolicies.GetEnumerator()) {
            New-ItemProperty -Path $yandexPolicy -Name $policy.Key -Value ([int]$policy.Value) -PropertyType DWord -Force | Out-Null
        }
        New-ItemProperty -Path $yandexPolicy -Name 'NtpWallpaper' -Value $backgroundPath -PropertyType String -Force | Out-Null
        $removedAutostartEntries = Disable-YandexBrowserAutostart
        $configured.Add('Яндекс Браузер: защита и простой режим')
        Write-ServiceLog "Яндекс Браузер: отключены автозапуск окна и фоновый перезапуск; удалено записей автозапуска Windows: $removedAutostartEntries. Также отключены уведомления новой вкладки, подсказки обучения, ротация/HD-анимация фона, боковая панель и лишние нейроэлементы; установлен спокойный статичный фон."

        Start-Process -FilePath $yandexCandidates[0] -ArgumentList @('--new-window',$adGuardUrl,$trafficLightUrl)
        $confirmation = [Windows.Forms.MessageBox]::Show("В Яндекс Браузере открыты две официальные страницы:`n`n1. AdGuard AdBlocker — убирает баннеры`n2. Bitdefender TrafficLight — предупреждает об опасных сайтах`n`nУстановите ОБА расширения, затем нажмите ОК.`nЗакладки, пароли и история не изменяются.",'Разовая установка защиты Яндекса','OKCancel','Information')
        if ($confirmation -eq 'OK') {
            Start-Sleep -Milliseconds 700
            $adGuardFound = Test-Path "C:\Users\*\AppData\Local\Yandex\YandexBrowser\User Data\*\Extensions\$adGuardId"
            $trafficLightFound = Test-Path "C:\Users\*\AppData\Local\Yandex\YandexBrowser\User Data\*\Extensions\$trafficLightId"
            $yandexExtensionsConfirmed = $adGuardFound -and $trafficLightFound
            if ($yandexExtensionsConfirmed) {
                Write-ServiceLog 'Яндекс Браузер: наличие AdGuard и TrafficLight подтверждено по профилю.'
            } else {
                $missing = @()
                if (-not $adGuardFound) { $missing += 'AdGuard AdBlocker' }
                if (-not $trafficLightFound) { $missing += 'Bitdefender TrafficLight' }
                Write-ServiceLog "Яндекс Браузер: не подтверждена установка: $($missing -join ', ')."
                [Windows.Forms.MessageBox]::Show("Не удалось подтвердить установку: $($missing -join ', ').`n`nНастройки защиты Яндекса уже применены. Оставшиеся расширения можно установить из открытых вкладок и затем повторно нажать кнопку №4.",'Нужно завершить установку','OK','Warning') | Out-Null
            }
        }
    }

    if ($configured.Count -eq 0) { throw 'Chrome и Яндекс Браузер не обнаружены.' }
    $resultText = "Настройки применены:`n`n• " + ($configured -join "`n• ")
    if ($chromeInstalled) { $resultText += "`n`nChrome установит AdGuard и TrafficLight после полного перезапуска браузера." }
    if ($yandexInstalled -and $yandexExtensionsConfirmed) { $resultText += "`nЯндекс Браузер: оба расширения подтверждены." }
    $resultText += "`n`nMicrosoft Defender и встроенная защита Protect не отключались."
    [Windows.Forms.MessageBox]::Show($resultText,'Защита браузеров настроена','OK','Information') | Out-Null
}
...
<div class="step safe"><h2>Шесть кнопок главного окна</h2><ol><li><b>Полное обслуживание</b> — рекомендуемый пошаговый цикл.</li><li><b>Диагностика и отчёт</b> — аудит и отправка в GitHub без изменений системы.</li><li><b>Очистка и ускорение</b> — очистка, оптимизация и еженедельная задача.</li><li><b>Защита браузеров</b> — проверка/замена стороннего антивируса, защита Chrome и Яндекс Браузера, упрощение их интерфейса.</li><li><b>Zapret и проверка YouTube</b> — установка стратегии №9 и контроль соединения.</li><li><b>Дополнительные инструменты</b> — редкие и потенциально опасные операции.</li></ol></div>
...
<div class="step"><span class="num">9. Настроить защиту браузеров</span><p>Для Chrome автоматически назначает совместимую пару: AdGuard убирает баннеры, Bitdefender TrafficLight предупреждает о фишинге и опасных сайтах. Также блокирует всплывающие окна, запросы уведомлений и опасные загрузки. В Яндекс Браузере отключает запуск окна вместе с Windows, фоновый перезапуск, рекламные информеры, обучение, вращение и HD-анимацию фона, боковую панель и лишние элементы новой вкладки. Компоненты обновления браузера сохраняются. Яндекс требует один раз подтвердить оба расширения на официальных страницах магазина — программа откроет их сама и проверит результат.</p></div>
<div class="step"><span class="num">10. Zapret: установить / обновить</span><p>Копирует или скачивает весь комплект Zapret в постоянную папку <code>C:\ProgramData\SeniorLaptopService\Zapret</code>, удаляет прежнюю службу и устанавливает стратегию №9 именно из этой папки. Затем проверяет путь запущенной службы и открывает тестовое видео в браузере по умолчанию, кроме Edge. После извлечения USB Zapret продолжает работать.</p></div>
<div class="step"><span class="num">11. Установить еженедельную очистку</span><p>Делайте после завершения настройки. Каждое воскресенье в 12:00 очистка запускается от SYSTEM и обновляет списки в постоянной папке Zapret. Повторное нажатие безопасно: задача заменяется новой версией.</p></div>
...
<h2>Рекомендуемая последовательность для обычного обслуживания</h2><p><b>Обновление → Аудит → Отчёт GitHub → Анализ → Очистка → исправление Яндекса/замена стороннего антивируса при необходимости → перезагрузка → Оптимизация → защита Chrome и Яндекса → Zapret → Еженедельная задача → Памятка.</b></p>
...
$securityWorkflow = {
    Write-ServiceLog 'WORKFLOW: antivirus check and browser protection started.'
    & $removeAntivirus
    & $installAdBlockAction
    Write-ServiceLog 'WORKFLOW: antivirus check and browser protection completed.'
}
...
Add-ActionButton '4. Защита браузеров' $securityWorkflow 1 1 ([Drawing.Color]::MistyRose)
