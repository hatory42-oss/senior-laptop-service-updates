...
$zapretUpdate = {
    ...
    [Windows.Forms.MessageBox]::Show("Zapret установлен и запущен.`n`nСтратегия №9: $($strategyResult.registered)`nСлужба: $($strategyResult.service_status)`n`nYouTube подтвердил соединение. Тестовое окно закрыто автоматически через 45 секунд. Системные звуки не изменялись.",'Проверка Zapret','OK','Information') | Out-Null
}
