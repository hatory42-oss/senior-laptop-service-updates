...
function Send-ReportToGitHub([string]$ReportPath, [string]$Token) {
    $repo = 'hatory42-oss/senior-laptop-service-reports'
    $safeComputer = ($env:COMPUTERNAME -replace '[^A-Za-z0-9._-]','_')
    $remotePath = 'reports/{0}/{1}/{2}-{3}.json' -f (Get-Date -Format 'yyyy'),(Get-Date -Format 'MM'),$safeComputer,(Get-Date -Format 'yyyyMMdd-HHmmss')
    ...
    $headers = @{ Authorization="Bearer $Token"; Accept='application/vnd.github+json'; 'X-GitHub-Api-Version'='2022-11-28'; 'User-Agent'='SeniorLaptopService/0.12' }
    try {
...
function Add-ActionButton([string]$Text, [scriptblock]$Action, [int]$Column, [int]$Row, [Drawing.Color]$Color = [Drawing.Color]::WhiteSmoke) {
    ...
    $button.Add_Click({
        try { $form.UseWaitCursor = $true; & $Action }
            ...
            [Windows.Forms.MessageBox]::Show("Операция завершилась ошибкой.`n`n$($_.Exception.Message)`n`nАвтоматический отчёт создан:`n$errorReportPath`n`nПуть скопирован в буфер обмена.",'Ошибка','OK','Error') | Out-Null
...
$reportAction = {
    ...
    $token = [Environment]::GetEnvironmentVariable('SLS_GITHUB_TOKEN','Process')
    if ([string]::IsNullOrWhiteSpace($token)) { $token = Show-PasswordPrompt 'Отправка отчёта в GitHub' 'Введите fine-grained GitHub token с доступом Contents: Read and write только к репозиторию senior-laptop-service-reports. Токен не сохраняется.' }
