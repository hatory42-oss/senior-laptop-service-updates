...
function Invoke-ResponsiveGitHubRequest([string]$Method, [string]$Uri, [string]$Token, [string]$JsonBody = $null, [int]$TimeoutSeconds = 45) {
    ...
    if ($null -ne $JsonBody) { $request.Content = [Net.Http.StringContent]::new($JsonBody,[Text.Encoding]::UTF8,'application/json') }
...
$reportAction = {
    ...
    Write-ServiceLog "Отчёт: JSON подготовлен ($($json.Length) символов)."
    $desktop = [Environment]::GetFolderPath('Desktop')
