...
function Invoke-ReliableZipDownload([string[]]$Urls, [string]$Destination) {
    ...
    for ($attempt=1; $attempt -le 3; $attempt++) {
        foreach ($downloadUrl in $Urls) {
            try {
                ...
                $downloadHeaders = @{ 'User-Agent'='SeniorLaptopService/0.21'; 'Accept' = if ($downloadUrl -like 'https://api.github.com/*') { 'application/octet-stream' } else { 'application/zip,application/octet-stream' } }
...
$zapretUpdate = {
    ...
    Invoke-ReliableZipDownload @([string]$asset.browser_download_url,[string]$asset.url) $zip
    ...
    foreach ($strategyNumber in $strategyOrder) {
        ...
        Start-Sleep -Seconds 3
        $streamStatus = Test-YouTubeMediaStream 'LNrBbGcLhXg' 25
