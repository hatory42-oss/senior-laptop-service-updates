...
$zapretUpdate = {
    if (-not (Test-Administrator)) { throw 'Запустите программу от имени администратора.' }
    Write-ServiceLog "Downloading pinned Zapret $PinnedZapretVersion from the service engineer repository..."
    ...
    Set-Content -LiteralPath (Join-Path $ZapretRoot '.installed-version') -Value $PinnedZapretVersion -Encoding ASCII
    Remove-Item $zip -Force -ErrorAction SilentlyContinue; Remove-Item $stage -Recurse -Force -ErrorAction SilentlyContinue
    Write-ServiceLog "Pinned Zapret $PinnedZapretVersion installed cleanly from hatory42-oss repository. Testing strategy #9 first."
